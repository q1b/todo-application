...
export const sampleOne = (arr: any[]) => {
...
export const getDistanceBetweenPoints = (p1: Coordinate, p2: Coordinate) => {
...
export const convertCartesianToPolar = (point, centerPoint = [0, 0]) => {
	const pointRelativeToCenter: [number, number] = [
...
export const getQuadrantForPoint = ([x, y]: [number, number]) => {
	if (x >= 0 && y >= 0) {
		return 1;
	} else if (x < 0 && y >= 0) {
		return 2;
 ...
 */
export const splitCommaSeparatedArray = (str: string) => {
